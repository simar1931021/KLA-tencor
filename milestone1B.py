...
def dispatch_flow(flow_name, activities):
    for task_name in activities.keys():
        ...
        if task.get('Type') == 'Task':
            if task.get('Execution') == 'Sequential':
                dispatch_sequential_tasks(flow_name, task_name, task)
            elif task.get('Execution') == 'Concurrent':
                dispatch_concurrent_tasks(flow_name, task_name, task)
...
def dispatch_concurrent_tasks(flow_name, task_name, task):
    log_task(flow_name, task_name, 'Entry')
    function_name = task.get('Function')
    function_args = ", ".join(list(task.get('Inputs').values()))
    execution_time = task.get('Inputs').get('ExecutionTime')
    x1 = Thread(target=log_task(flow_name, task_name, f'Executing {function_name}({function_args})'))
    x1.start()
    time.sleep(float(execution_time))
    x2 = Thread(target = (log_task(flow_name, task_name, 'Exit'))
    x2.start()
    x1.join()
    x2.join()
